...
int TCPObj::traceGlobalIndex = 0;	//used for trace memory  
...
int TCPObj::InsertTCB(TCBRecord *tcb)
{
   ...
   FreeTCB->traceIndex=tcb->traceIndex;
...
//****************************************************************************
// Delete a Row in TCB
int TCPObj::DeleteTCB(int TCBRecordNum, int retpoint)
{
   long prev;
   long next;
   long hashValue =0;
   ...
   {
      storeTraceDword((io.AOAgetTimer() - tcb->Rsyn)/4, tcb->traceIndex, 7); //time in milli-secs   
      storeTraceDword(0xDDDDDDDD, tcb->traceIndex, 15); //tcb deleted   
      storeTraceDword(tcb->state, tcb->traceIndex, 4); //tcb deleted   
      storeTraceDword(tcb->http_state, tcb->traceIndex, 17); //tcb deleted   
      storeTraceDword(retpoint, tcb->traceIndex, 14); //deletetcb return point    
   }

   io.AOAprintHex(traceGlobalIndex, Line24+20); 
   TotalDelCount++; 
...
if( TCBRecordNum != -1 )
{
   ...
   {        
	  ...
	  if (TRACE_RECORD_FLAG) 
        storeTraceDword(0xC0000001, tcb->traceIndex, 72); //client reset  

      NoOfResets++; 
      if (tcb->taskExists == 0) //RKK & BHARAT  
      {
         io.AOAPrintText("Reset here new code 1", Line24+20); 
         tcb->resetFlag = 1; 
         return -45; 
      ...
      {
          //RKK-CHANGE 10-21-2014
          //when a client sends reset, resume the task and remove it 
          if (tcb->suspendFlag == 1) 
          {
              io.AOAPrintText("Reset here new code 2", Line24+20); 
              tcb->resumeFlag = 1; 
              tcb->resetFlag = 1; 
              return -49; 
          }
          else 
             DeleteTCB(TCBRecordNum, 0x00000001);
      }
      TCBRecordNum = -1; 
      return -48;   //RKK & BHARAT 
   }
}
				...
				char *macaddr, long timer, int currenttask)
{
	...
	unsigned long tempseq=0;
		...
		temptcb.Rfina=0;
		temptcb.Rfinaa=0;
        traceGlobalIndex++;  //increment the global index for trace  
   		temptcb.traceIndex=traceGlobalIndex;
		...
		else
		{
			...
			tempseq=charToseqnum(&TCPPack[0]);
	        if (TRACE_RECORD_FLAG) 
               {
                 storeTraceDword(0xA0000001, tcb->traceIndex, 48); //send ack 
                 storeTraceDword(TCBRecordNum, tcb->traceIndex, 0); //store SYN in the first entry 
                 storeTraceDword(tempseq, tcb->traceIndex, 5); //store portno  
               }
	      return retcode;
	     }
	  }// End of SYN 
	...
	if((TCPPack[13] & FINACK) == FINACK) 
	{
	     ...
	     if (TRACE_RECORD_FLAG) 
         {
           storeTraceDword(0xFFFFFFFF, tcb->traceIndex, 40); //dummy finack came 
         }
	}
	...
	// 2. Check for TACK
	 ...
	 // 3. Check for TFIN
	      ...
	      retcode = SendMisc(SourceIP, &TCPPack[0], TRST, macaddr, RESETTYPE, currenttask);
	      if (TRACE_RECORD_FLAG) 
              storeTraceDword(0xE0000002, tcb->traceIndex, 73); //server reset  
          //return retcode;
        }
...
int TCPObj::OtherHandler(char* TCPPack,int size, int TCBRecordNumber, long timerEth, int currenttask)
{
    ...
    {	
		...
		//if(tcb->suspendFlag == 1)
	    if (TRACE_RECORD_FLAG) 
          storeTraceDword(0xC0000002, tcb->traceIndex, 72); //client reset  
        NoOfResets++; 
		if(tcb->taskExists == 1)
		{
		    tcb->resetFlag=1;
        ...
        {
            if (tcb->taskExists == 0)   //RKK & BHARAT
		       DeleteTCB(TCBRecordNumber, 0x00000002); 
        }
		return 0;	
	    }
	    else
...
//			io.AOAprintHex((long)tcb->PORT, Line22+120);
	        ...
	        if (TRACE_RECORD_FLAG) 
              storeTraceDword(0xE0000003, tcb->traceIndex, 73); //server reset  
//	         io.AOAPrintText("ii",Line19);
			...
			if(tcb->taskExists == 1)
			{
                ...
                {
		            DeleteTCB(TCBRecordNumber, 0x00000003); 	
                }
      ...
      if((tcp->Flags & TRST) == TRST)
	    {
	      ...
	      //if(tcb->suspendFlag == 1)
	      if (TRACE_RECORD_FLAG) 
             storeTraceDword(0xC0000003, tcb->traceIndex, 72); //client reset  
          NoOfResets++; 
          ...
          {
            if (tcb->taskExists == 0) 
		      DeleteTCB(TCBRecordNumber, 0x00000004); 
          }
	     return 0;
	    }
	...
	if((tcp->Flags & TSYN) == TSYN)
	{
	    ...
	    if (TRACE_RECORD_FLAG) 
           storeTraceDword(0xA0000002, tcb->traceIndex, 49); //send ack 
//	    io.AOAPrintText("jj",Line19);
	...
	if((tcp->Flags & TACK) == TACK)
	{
	    ...
	    if (TRACE_RECORD_FLAG) 
           storeTraceDword(tcb->SNDWND, tcb->traceIndex, 23);  
	   ...
	   if(tcb->state == SYNRCVD)
        ...
        if (TRACE_RECORD_FLAG)
          storeTraceDword(0x0041434b, tcb->traceIndex, 1); //store SYN ACK in the first entry 
		   ...
		   (greaterEqualto(tcb->SNDNXT, tcp->acknumber) >= 0) )
		{
		    ...
		    tcb->state= ESTAB;
           if (TRACE_RECORD_FLAG)
            storeTraceDword(tcb->state, tcb->traceIndex, 2); //ESTAB state
...
//			io.AOAprintHex(max, Line3+140);
		    ...
		    if(len !=0)
		    {
			   if(TCPPack[20] == 'G' && TCPPack[21] == 'E' && TCPPack[22] == 'T')
			   {
                ...
                if (TRACE_RECORD_FLAG)
                  storeTraceDword(0x00474554, tcb->traceIndex, 3); //GET 
...
//						tcb->IP, tcb->PORT);
			      ...
			      //}			      
	                      ...
	                      if (TRACE_RECORD_FLAG) 
                             storeTraceDword(0xA0000003, tcb->traceIndex, 50); //send ack 
...
//	                io.AOAprintHex(tcp->acknumber,Line19+140);
	        ...
	        if (TRACE_RECORD_FLAG) 
              storeTraceDword(0xE0000004, tcb->traceIndex, 73); //server reset  
...
//			    io.AOAPrintText("EE", Line19+100);
                ...
                {
		            DeleteTCB(TCBRecordNumber, 0x00000005); 
                }
			return retcode;
		}
	    } // end of if SYNRCVD
	    ...
	    if(tcb->state == ESTAB)
	    {
            //RKK-ALW October 10, 2014 
            //problem fixed is the following 
            //server sent 16k data packets as the window size of client was 16k 
            // client sent all acks acknowleding 16k data, but server sent a reset to client 
            // tcb->SNDNXT was not equal to tcb->SNDUNA as it may not be updated!!!!
            // Let us try this fix
			tcb->SNDUNA = tcp->acknumber; //this fix worked from home 

        if (TRACE_RECORD_FLAG) 
         {
          storeTraceDword(tcb->state, tcb->traceIndex, 16); //ESTAB state parameters  
          storeTraceDword(tcb->http_state, tcb->traceIndex, 17); 
          storeTraceDword(tcb->SNDNXT, tcb->traceIndex, 18); 
          storeTraceDword(tcb->SNDUNA, tcb->traceIndex, 19); 
          storeTraceDword(tcp->acknumber, tcb->traceIndex, 20); 
          storeTraceDword(tcp->seqnumber, tcb->traceIndex, 21); 
         }
        //RKK-AW if there is a connection, but no GET and no task running then 
        //  send ack and close, client wants to close connection 
         if((TCPPack[13] & FINACK) == FINACK) 
	       {
             //DUMMY-FINACK
             if (tcb->suspendFlag == 0 && tcb->taskExists == 0)
             {
               retcode = SendMisc(TCPPack, tcb->IP, tcb->destmac, currenttask);
	           if (TRACE_RECORD_FLAG) 
                  storeTraceDword(0x05DFAC01, tcb->traceIndex, 40); //dummy finack came 
               DeleteTCB(TCBRecordNumber, 0x00000006); 
               return 0; 
             }
	       }
		   ...
		   (greaterEqualto(tcb->SNDNXT, tcp->acknumber) >= 0))	
		{
            if (TRACE_RECORD_FLAG) 
            {
               storeTraceDword(0x05CACE01, tcb->traceIndex, 9); //ESTAB state case 1 
               storeTraceDword(0x05, tcb->traceIndex, 4); //state 
            }
...
//	                io.AOAprintHex(tcb->EXPHEADACK, Line5);	
		          ...
		          if(tcb->suspendFlag==1)        //let the task get out of delay
                            {
                              ...
                              if (TRACE_RECORD_FLAG) 
                               {
                                storeTraceDword(0x05DDDDDD, tcb->traceIndex, 10); //ESTAB state all data reached by client 
                                storeTraceDword(0x05, tcb->traceIndex, 4); //state 
                                storeTraceDword(tcb->suspendFlag, tcb->traceIndex, 11);  
                                storeTraceDword(tcb->resumeFlag, tcb->traceIndex, 12); 
                                storeTraceDword(tcb->Sent_Flag, tcb->traceIndex, 31); 
                               }
                            }
			...
			if((tcp->Flags & TFIN) == TFIN)
			{
			    ...
			    if (TRACE_RECORD_FLAG) 
                               {
                                  storeTraceDword(0x05000001, tcb->traceIndex, 41); //finack came 
                                  storeTraceDword(0x05, tcb->traceIndex, 4); //state 
                               }

			    tcb->state = CLOSEWAIT;
			    if (TRACE_RECORD_FLAG) 
                               storeTraceDword(tcb->state, tcb->traceIndex, 4); //state 
	  	 	    ...
	  	 	    retcode =  SendMisc(tcb->IP, tcb->PORT, ACKTYPE, tcb->destmac, ACKTYPE, currenttask);
	                    if (TRACE_RECORD_FLAG) 
                              storeTraceDword(0x05FAC004, tcb->traceIndex, 51); //ack 
			    ...
			    {
		          DeleteTCB(TCBRecordNumber, 0x00000007); 
			    }
                            return 0;
			}
			      ...
			      if (tcb->getackflag == 1)  //RKKAW Duplicate GET came in 
                               {
                                 ...
                                 retcode =  SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, GETACKTYPE, currenttask);
	                             if (TRACE_RECORD_FLAG) 
                                   storeTraceDword(0xA0000005, tcb->traceIndex, 52); //ack 
                                 return 0; 
                               }
...
//						tcb->IP, tcb->PORT);
	              ...
	              retcode =  SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, GETACKTYPE, currenttask);
	              if (TRACE_RECORD_FLAG) 
                     storeTraceDword(0xA0000006, tcb->traceIndex, 53); //ack GET ACK  
                  tcb->Rgeta=io.AOAgetTimer();
                  tcb->getackflag=1;
			      if(retcode != 0)
			       {
...
//				     io.AOAprintHex(retcode, Line23+80); 
        ...
        //
               ...
               else if (tcb->partialFlag == 1) //No GET in this packet  
                {
	              ...
	              retcode =  SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, GETACKTYPE, currenttask);
	              if (TRACE_RECORD_FLAG) 
                     storeTraceDword(0xA0000007, tcb->traceIndex, 54); //ack 
                  if (tcb->partialFlag == 0) //this is a complete packet, start the task to process GET 
                     {
			           HttpCount++; //increment the http count  
			           HttpRequests++;  //total no of HTTP requests 
...
//		PrintTCB(TCBRecordNumber, Line18);
		...
		//	tcb->LST = io.AOAgetTimer();
			    if (TRACE_RECORD_FLAG) 
                {
                   storeTraceDword(0x05CACE02, tcb->traceIndex, 9); //ESTAB state case 2 
                   storeTraceDword(0x05, tcb->traceIndex, 4); //state 
                }
			return 0;
		}
...
//			PrintTCB(TCBRecordNumber,Line19);
			    if (TRACE_RECORD_FLAG) 
                {
                   storeTraceDword(0x05CACE03, tcb->traceIndex, 9); //ESTAB state case 3 
                   storeTraceDword(0x05, tcb->traceIndex, 4); //state 
                }

	  		retcode =  SendMisc(tcb->IP, tcb->PORT, TRST, tcb->destmac, RESETTYPE, currenttask);
	        if (TRACE_RECORD_FLAG) 
              storeTraceDword(0xE0000005, tcb->traceIndex, 73); //server reset  
...
//			   		    io.AOAPrintText("FF", Line19+100);
                ...
                {
		           DeleteTCB(TCBRecordNumber, 0x00000008); 
                }
        ...
        //			PrintTCB(TCBRecordNumber,Line20);
			    if (TRACE_RECORD_FLAG) 
                {
                   storeTraceDword(0x05CACE04, tcb->traceIndex, 9); //ESTAB state case 4 else case  
                   storeTraceDword(0x05, tcb->traceIndex, 4); //state 
                }
		}
          ...
          if (((tcb->EXPRESACK + 1) == tcb->SNDNXT)) 
             {
                 ...
                 if (TRACE_RECORD_FLAG) 
                   {
                     storeTraceDword(tcb->state, tcb->traceIndex, 24); //FINWAIT state parameters  
                     storeTraceDword(tcb->http_state, tcb->traceIndex, 25); 
                     storeTraceDword(tcb->SNDNXT, tcb->traceIndex, 26); 
                     storeTraceDword(tcb->SNDUNA, tcb->traceIndex, 27); 
                     storeTraceDword(tcp->acknumber, tcb->traceIndex, 28); 
                     storeTraceDword(tcp->seqnumber, tcb->traceIndex, 29); 
                     storeTraceDword(tcb->EXPRESACK+1, tcb->traceIndex, 30); 
                   }
             }
		...
		{
                   storeTraceDword(tcb->state, tcb->traceIndex, 32);//acknumber is equal sndnxt  
                   storeTraceDword(tcp->acknumber, tcb->traceIndex, 33);//acknumber  
                   storeTraceDword(tcb->SNDNXT, tcb->traceIndex, 34);//sndnxt  
		   tcb->SNDUNA = tcp->acknumber;
                      //tcb->LST = io.AOAgetTimer();
	                   //		tcb->SRTT = CalcRTT(tcb->LST,tcb->SRTT);
                       // tcb->LST = io.AOAgetTimer();
          ...
          if((tcp->Flags & TFIN) == TFIN)
			{
			    ...
			    tcb->Rfina=timerEth;
			    if (TRACE_RECORD_FLAG) 
                  storeTraceDword(0x06000002, tcb->traceIndex, 41); //finack came 
	         	retcode =  SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, FINACKTYPE, currenttask); 
	            if (TRACE_RECORD_FLAG) 
                  storeTraceDword(0x06FAC008, tcb->traceIndex, 55); //ack 
           ...
           if((tcp->Flags & TFIN) == TFIN)
			{
	            ...
	            if (TRACE_RECORD_FLAG) 
                     storeTraceDword(0x06FACA09, tcb->traceIndex, 56); //ack 

			    if (TRACE_RECORD_FLAG) 
                   storeTraceDword(0x06FAC003, tcb->traceIndex, 41); //finack  
			    if(retcode!=0)
			        return retcode;			    
//			io.AOAPrintText("cc", Line19);
			    ...
			    if(tcb->suspendFlag == 1)
			    {
		                ...
		                DeleteTCB(TCBRecordNumber, 0x00000009); 
...
//				    io.AOAPrintText("II", Line21+80);
                     ...
                     {
		                DeleteTCB(TCBRecordNumber, 0x00000010); 
                     }
	                     return 0;	
		        }
                else 
                      ...
                      if (size == 21) //keep-alive packet  
                        {
	                      ...
	                      if (TRACE_RECORD_FLAG) 
                             storeTraceDword(0x21000010, tcb->traceIndex, 57); //ack 
                        }
                       else      
                       {
	                       ...
	                       if (TRACE_RECORD_FLAG) 
                             storeTraceDword(0xA0000011, tcb->traceIndex, 58); //ack 
                             */
...
/*	    
	   ...
	   {	
		...
		{
		   DeleteTCB(TCBRecordNumber, 0x00000011); 
	           return 0;	
		}
	        return 0;
	   }
*/
	   ...
	   {	
		if((tcp->Flags & TFIN) == TFIN)
		{
	        ...
	        retcode =  SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, FINACKTYPE, currenttask);  
	        if (TRACE_RECORD_FLAG) 
              storeTraceDword(0x09FAC012, tcb->traceIndex, 59); //fin ack in closing  
		    tcb->state = CLOSEWAIT; 
		    if(tcb->suspendFlag == 1)
		    {
		       tcb->Sent_Flag = 1;
	       ...
	       {
	         if (TRACE_RECORD_FLAG) 
               storeTraceDword(0xC0000004, tcb->traceIndex, 72); //client reset  
             NoOfResets++; 
		     if(tcb->taskExists == 1)
		       {
		         tcb->resetFlag=1;
               ...
               {
                  if (tcb->taskExists == 0)   //RKK & BHARAT
		          DeleteTCB(TCBRecordNumber, 0x00000012); 
               }
		     return 0;	
	       }
...
//		   //	    io.AOAPrintText("KK", Line20+80);
		     ...
		     {
		     DeleteTCB(TCBRecordNumber, 0x00000013); 
		     }
	           return 0;	
		}
		else
...
/*	   if(tcb->state == LASTACK)
	   {
		...
		if(tcp->acknumber == tcb->SNDNXT)
		{
			...
			DeleteTCB(TCBRecordNumber, 0x00000014);
			tcb->state = CLOSED;
		}
	        return 0;	
	   }
	...
	if((tcp->Flags & TFIN) == TFIN)
	{
	   ...
	   if(tcb->state == ESTAB)
	   {
		//io.AOAprintCharacter('E', Line20+120);
	  	...
	  	retcode =  SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, FINACKTYPE, currenttask);
	    if (TRACE_RECORD_FLAG) 
          storeTraceDword(0xA0000013, TCBRecordNumber, 41); //ack 
		tcb->state = CLOSEWAIT;
		return retcode;
	   }
	   ...
	   if(tcb->state == FINWAIT1)
	   {
        ...
        if(tcb->suspendFlag==1)        //let the task get out of delay
        {
	  	...
	  	retcode =  SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, FINACKTYPE, currenttask);
	    if (TRACE_RECORD_FLAG) 
          storeTraceDword(0xA0000014, tcb->traceIndex, 61); //ack 
...
//		    		    io.AOAPrintText("LL", Line19+100);
		    ...
		    DeleteTCB(TCBRecordNumber, 0x00000015); 
	   ...
	   if(tcb->state == FINWAIT2)
	   {
        ...
        if(tcb->suspendFlag==1)        //let the task get out of delay
        {
	  	...
	  	retcode =  SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, FINACKTYPE, currenttask);
	    if (TRACE_RECORD_FLAG) 
          storeTraceDword(0xA0000015, tcb->traceIndex, 62); //ack 
...
//		    		    io.AOAPrintText("MM", Line19+100);
		    ...
		    DeleteTCB(TCBRecordNumber, 0x00000016); 
...
/*	
	...
	if((tcb->state == ESTAB) || (tcb->state == FINWAIT1) || (tcb->state == FINWAIT2))
	{
		...
		if(len !=0)
		{
				...
				DeleteTCB(TCBRecordNumber, 0x00000017);
...
int TCPObj::SendMisc(char *packet, char *destIP, char *TargetMAC, int currenttask) 
{
    ...
    io.AOAprintHex((long)packet[0], Line20+20); 
    io.AOAprintHex((long)packet[1], Line20+40); 
    for (i=0; i<4; i++) 
      send_buffer[4+i] = packet[8+i]; //sequence number 
    for (i=0; i<4; i++) 
      send_buffer[8+i] = packet[4+i]; //ack number
    j = 0; 
    k = 0; 
    j = send_buffer[8];
    j = j << 24; 
	...
	send_buffer[8] = (j>>24)&0x000000FF;
	send_buffer[9] = (j>>16)&0x000000FF;
	send_buffer[10] = (j>>8)&0x000000FF;
	send_buffer[11] = j&0x000000FF;
...
int TCPObj::SendN(int TCBRecordNum, int currenttask)
{
  	 			    ...
  	 			    // one stored in tcb table by receive task
   {
	   ...
	   if (TRACE_RECORD_FLAG) 
         storeTraceDword(0xE0000006, tcb->traceIndex, 73); //server reset  
       if(retcode!=0)
           return retcode;
//	io.AOAPrintText("dd", Line19);
 
       DeleteTCB(TCBRecordNum, 0x00000018);		 
...
int TCPObj::tcpSendN(int TCBRecordNum, int currenttask)
{
	...
	while (tcb->http_complete == 0)
	   ...
	   switch (tcb->http_state) 
		...
		case HTTP_SENDP: 
            if (PRINT)
             ...
             //sliding window is bigger than the data to send 
		     if(tcb->SNDWND >= (tcb->HSIZE + tcb->FILESIZE) && tcb->first_send == 0)
	           {
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACE0001, tcb->traceIndex, 80); //case1  
		       tcb->http_case = 0x01; 
               ...
               //
               //sliding window is smaller than the header size 
		   else if(tcb->SNDWND < tcb->HSIZE && tcb->first_send == 0)
	            {
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACE0002, tcb->traceIndex, 81); //case2  
		      tcb->http_case = 0x02; 
		      tcb->first_send = 1; 
              if(tcb->SNDWND ==0) //RKK-AW window is zero do not send ACK 
	                {
	                 ...
	                 if (TRACE_RECORD_FLAG) 
                       storeTraceDword(0xA0000018,tcb->traceIndex, 63); //ack 
	                 if(retcode !=0)
			          {
			             tcb->http_state = HTTP_EXIT; 
			             DeleteTCB(tcbrno, 0x00000022); 
	                     return retcode; 
			          }
                      */
	                }
               ...
               //sliding window is greater than header and some data 
		   else if(tcb->SNDWND > tcb->HSIZE && tcb->SNDWND < (tcb->HSIZE + tcb->FILESIZE) && tcb->first_send == 0)
	            {
                 if (TRACE_RECORD_FLAG) 
                    storeTraceDword(0xCACE0003, tcb->traceIndex, 82); //case3  
		         tcb->http_case = 0x03; 
		         tcb->first_send = 1; 
	             retcode = sendHeader1(tcbrno, tcb->SEQ, tcb->EXPHEADACK, tcb->CURRENTTASK);
	             if(retcode != 0)
	           ...
	           //startHeaderSeq  ack           startResourceSeq           expectedResourceAck 
			     ...
			     {
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACE0004, tcb->traceIndex, 83); //case4  
		           tcb->http_case = 0x04; 
	                  if(tcb->SNDWND ==0)
	                  {
                        /* RKK-AW
	                    retcode=SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, SW0TYPE, tcb->CURRENTTASK);
	                    if (TRACE_RECORD_FLAG) 
                          storeTraceDword(0xA0000017, tcb->traceIndex, 64); //ack 
			            if(retcode !=0)
			              {
			               tcb->http_state = HTTP_EXIT; 
			               DeleteTCB(tcbrno, 0x00000026); 
			               return retcode; 
			             }
                         */
	                  }
			  else
			  {
	           ...
	           //startHeaderSeq  ack           startResourceSeq           expectedResourceAck 
			    ...
			    {
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACE0005, tcb->traceIndex, 84); //case5  
		            tcb->http_case = 0x05; 
			    tcb->k1 = tcb->SEQ + tcb->k;
		  ...
		  //                 |<---------sliding window-------->|    
			...
			{
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACE0006, tcb->traceIndex, 85); //case6  
		           tcb->http_case = 0x06; 
			   tcb->k1 = tcb->SEQ + tcb->k;
			   retcode = sendHeader1(tcbrno, tcb->k1, tcb->EXPHEADACK, tcb->CURRENTTASK);  
			   if(retcode != 0)
		   ...
		   //                                   |<---------sliding window-------->|    
		    ...
		    {
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACE0007, tcb->traceIndex, 86); //case7  
		        tcb->http_case = 0x07; 
			...
			if(retcode != 0)
			  {
			       tcb->http_state = HTTP_EXIT; 
			       DeleteTCB(tcbrno, 0x00000032); 
			       return retcode;
			  }
		    }

		   //ack is within the payload Case 8
           //sliding window is smaller than the total data to be sent 
		   //                                   |<-----sliding window---->|    
	           //  |<-----hSize------->|<-----------------fileSize------------------->|
	           //  |-------------------|------------|---------------------------------|
		    ...
		    { 
                   if (TRACE_RECORD_FLAG) 
                   {
                      storeTraceDword(0xCACE0008, tcb->traceIndex, 87); //case8  
                      storeTraceDword(tcb->state, tcb->traceIndex, 4);  
                      storeTraceDword(tcb->state, tcb->traceIndex, 16);  
                      storeTraceDword(tcb->http_state, tcb->traceIndex, 17);  
                   }
		          tcb->http_case = 0x08; 
	              ...
	              if(tcb->SNDWND ==0)
	              {
	                ...
	                retcode=SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, SW0TYPE, tcb->CURRENTTASK);	
	                if (TRACE_RECORD_FLAG) 
                        storeTraceDword(0xA0000018, tcb->traceIndex, 65); //ack 
		            if(retcode !=0 )
			         {
			            tcb->http_state = HTTP_EXIT; 
			            DeleteTCB(tcbrno, 0x00000033); 
		                return retcode; 
			         }
                     */
	              } 
		...
		case HTTP_RETRP: 
		        ...
		        if(mod32cmp(io.AOAgetTimer(), tcb->LOOPCOUNT) > TIMER_PERIOD*1091 )
		          {
		            retcode =  SendMisc(tcb->IP, tcb->PORT, TRST, tcb->destmac, RESETTYPE, currenttask);
	                if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xE0000007, tcb->traceIndex, 73); //server reset  
		            tcb->http_state = HTTP_EXIT; 
			    DeleteTCB(tcbrno, 0x00000035); 
			    return retcode; 
		          }
		   ...
		   //                 |<--sliding window-->|      
			...
			{
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACEE001, tcb->traceIndex, 74); //case1  
		          tcb->http_case = 0x11; 
	                  if(tcb->SNDWND ==0)
	                  {
	                    retcode=SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, SW0TYPE, tcb->CURRENTTASK);
	                    if (TRACE_RECORD_FLAG) 
                          storeTraceDword(0xA0000019, tcb->traceIndex, 64); //ack 
			    if(retcode !=0)
			       {
			         tcb->http_state = HTTP_EXIT; 
			         DeleteTCB(tcbrno, 0x00000036); 
			         return retcode; 
			       }
	                  }
			  else
		   ...
		   //                 |<----------------sliding window----------------------->|    
			...
			{
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACEE002, tcb->traceIndex, 75); //case2  
		            tcb->http_case = 0x12; 
			    tcb->k1 = tcb->SEQ + tcb->k;
			    retcode = sendHeader1(tcbrno, tcb->k1, tcb->EXPHEADACK, tcb->CURRENTTASK);  
			    if(retcode != 0)
		  ...
		  //                 |<---------sliding window-------->|    
			...
			{
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACEE003, tcb->traceIndex, 76); //case3  
		           tcb->http_case = 0x13; 
			   tcb->k1 = tcb->SEQ + tcb->k;
			   retcode = sendHeader1(tcbrno, tcb->k1, tcb->EXPHEADACK, tcb->CURRENTTASK);  
			   if(retcode != 0)
		   ...
		   //                                   |<---------sliding window-------->|    
		    ...
		    {
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACEE004, tcb->traceIndex, 77); //case4  
		        tcb->http_case = 0x14; 
			tcb->k1 = tcb->STARTRESSEQ + tcb->k;
		        retcode = sendPayload4 (tcbrno, tcb->k1, tcb->CURRENTTASK);
			if(retcode != 0)
		   ...
		   //                                   |<-----sliding window---->|    
		    ...
		    { 
                   if (TRACE_RECORD_FLAG) 
                      storeTraceDword(0xCACEE005, tcb->traceIndex, 78); //case5  
		      tcb->http_case = 0x15; 
	              if(tcb->SNDWND ==0)
	              {
	                retcode=SendMisc(tcb->IP, tcb->PORT, TACK, tcb->destmac, SW0TYPE, tcb->CURRENTTASK);	
	                if (TRACE_RECORD_FLAG) 
                          storeTraceDword(0xA0000020, tcb->traceIndex, 65); //ack 
		        if(retcode !=0 )
			  {
			    tcb->http_state = HTTP_EXIT; 
			    DeleteTCB(tcbrno, 0x00000043); 
		            return retcode; 
			  }
	              }
		      else
       ...
       if(tcb->suspendFlag==1 && tcb->resetFlag==1)
	      {
            ...
            retcode =  SendMisc(tcb->IP, tcb->PORT, TRST, tcb->destmac, RESETTYPE, tcb->CURRENTTASK);
	        if (TRACE_RECORD_FLAG) 
                  storeTraceDword(0xE0000008, tcb->traceIndex, 73); //server reset  
  	        tcb->http_state = HTTP_EXIT; 
	        DeleteTCB(tcbrno, 0x00000045); 
	        return -39; 
	      }
	   ...
	   if(tcb->CMPVAL1 != 0xFFFFFFFF) // we get the last bit acknowledgement, transmit is finished
       {
	         tcb->Sent_Flag =1;
	         if (TRACE_RECORD_FLAG) 
                storeTraceDword(tcb->Sent_Flag, tcb->traceIndex, 31); //server reset  
       }
       ...
       if (tcb->SNDNXT == tcb->SNDUNA && tcb->Sent_Flag == 1) 
	        {
                ...
                { 
                   storeTraceDword(tcb->Sent_Flag, tcb->traceIndex, 31);
                   storeTraceDword(tcb->SNDNXT, tcb->traceIndex, 34); 
                   storeTraceDword(tcb->SNDUNA, tcb->traceIndex, 35); 
                   storeTraceDword(tcb->state, tcb->traceIndex, 32); 
                   storeTraceDword(tcb->http_state, tcb->traceIndex, 25); 
                }
		        if( tcb->state == CLOSING )
		          {
	   ...
	   else 
	     {
	            ...
	            if (TRACE_RECORD_FLAG) 
		         {
                    storeTraceDword(0xE0000009, tcb->traceIndex, 73); //server reset  
                    storeTraceDword(tcb->SNDWND, tcb->traceIndex, 23);  
                    storeTraceDword(tcb->http_state, tcb->traceIndex, 25);  
                    storeTraceDword(tcb->SNDNXT, tcb->traceIndex, 26);  
                    storeTraceDword(tcb->SNDUNA, tcb->traceIndex, 27);  
    ...
    DeleteTCB(tcbrno, 0x00000046); 
	if (TRACE_RECORD_FLAG) 
	  {
       storeTraceDword(tcb->http_complete, tcb->traceIndex, 36);  
      }
...
int TCPObj::sendPayload3 (int TCBRecordNum,  unsigned long seqnum, int currenttask)
{
    ...
    {
      storeTraceDword(tcb->state, tcb->traceIndex, 4);  
      storeTraceDword(tcb->noOfPackets, tcb->traceIndex, 42);  
      storeTraceDword(tcb->leftOverBytes, tcb->traceIndex, 43);  
    }
...
int TCPObj::sendPayload4 (int TCBRecordNum, unsigned long seqnum, int currenttask)
{
    ...
    {
      storeTraceDword(tcb->state, tcb->traceIndex, 4);  
      storeTraceDword(tcb->noOfPackets, tcb->traceIndex, 44);  
      storeTraceDword(tcb->leftOverBytes, tcb->traceIndex, 45);  
    }
...

{
   ...
   if (traceGlobalIndex < MAX_TRACE3_COUNT)
	{
        addr = addr + base*512+index*4; 
        p1 = (int*)addr; 
        *p1 = dataw; 
   ...
   {
       io.AOAPrintText("Trace Global Index Exceeded!", Line24+20); 
   }
   return 0; 
}; 

...
int parserObj::processRequest(int currenttcbrno, int currenttask)
	{	
		...
		if(requestMsg[0]=='\0')
        {
           io.AOAprintHex((long)requestMsg, Line21+20); 
...
int parserObj::methodGET(char *requestMsg,int currenttcbrno, int currenttask)	
	...
	{ 
		//wlst.insert(currenttask, tsk.Current_Tcbrno, 0x0f000000, 2); 
        io.AOAprintHex((long)requestMsg, Line20+20); 
		return -3;	
	}
	...
	if(retcode==0)  //parsing ends here
	{
        ...
        if (TRACE_RECORD_FLAG) 
            tcp.storeTraceDword(retcode, currenttcbrno, 6); //return code for tcpSendN  
...
int parserObj::createResponse(char *resourceName, char *cType, char *responseHeader,long *headerSize, unsigned long *fileStartAdress,unsigned long *fileSize)
	...
	char *statusCode;
    char debugs[5]; //2 digit numbers only 
    int debugindex = 0; 
    ...
    if (resourceName[0] =='d' && resourceName[1]=='e' && resourceName[2]=='b' && resourceName[3]=='u'
            && resourceName[4]=='g' && resourceName[7]=='.' &&  resourceName[8]=='h'&&  resourceName[9]=='t' && resourceName[10]=='m' ) //debug file name 
    {
        debugs[0]='0';
        debugs[1]='0';
        debugs[2]=resourceName[5];
        debugs[3]=resourceName[6]; //must be debug.xx format only two digit type 
        debugs[4]= '\0'; 
        debugindex =  io.AOAconvertChar4ToLong(debugs); //convert to int value 

	    retcode = h1.getFile("debugxx.html", &httpfile, &httpfsize);

        //-------------------------------------------------------------------------------------
        //during file transfer, debugxx.html is saved in memory at DEBUGTRACE_ADDR
        // every time a debug screen is sent, this template is filled with appropriate data 
        //  from debug windows which are 512 bytes in data 
        //  RKK October 15, 2014
        //-------------------------------------------------------------------------------------
        httpptr = (unsigned char *)(DEBUGTRACE_ADDR - ADDR_OFFSET);
        httpptr1 = (unsigned char *) httpfile; 
        for (i=0; i<httpfsize; i++)
           httpptr1[i] = httpptr[i]; //restore original debug template    

        //debug screen address as below 
        faddr = (TRACE3 - ADDR_OFFSET + debugindex*512); 
        retcode = copyTraceHtml(faddr, httpfile, httpfsize); 
        *fileStartAdress = httpfile; 
        *fileSize = httpfsize; 
        //debug files to access remotely  
    }
    else 
...
/****************************************************************************************/
//this copies trace data to html file 
/****************************************************************************************/
int parserObj::copyTraceHtml(unsigned long f1, unsigned long f2, unsigned long len)
{
    int i=0; 
    unsigned long *inptr; 
    unsigned char *outptr; 
    int count=0; 
    unsigned long v1 = 0x0a1b2c3d; 
    char ptr[10]; 

    inptr = (unsigned long *)f1; 
    outptr = (unsigned char *)f2; 

            //io.AOAconvertLongTo8HexChars(v1,ptr); 
    i=0; 
    while (i < len)
    {
        if (outptr[i] == 0x52 && outptr[i+1] == 0x4f && outptr[i+2] == 0x57 && outptr[i+3] ==0x58) 
        {
            io.AOAconvertLongTo8HexChars(inptr[count],(char *) &outptr[i]); 
            //convert long to 8 hex digits and store in the output 
            count++; 
            i = i + 8; 
        }
        else i++; 

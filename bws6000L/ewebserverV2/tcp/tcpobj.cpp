...
int TCPObj::DeleteTCB(int TCBRecordNum)
{
           ...
           || tcb->Rfinaa == 0 || tcb->Rgeta==0) && (tcb->taskExists != 0)) 
   {
       ...
       {
       /*
       storeTraceDword(TCBRecordNum);
       storeTraceDword(tcb->PORT[0]);
       storeTraceDword(tcb->PORT[1]);
       storeTraceDword(tcb->state);
       storeTraceDword(0);
       storeTraceDword(0);
       storeTraceDword(0);
       storeTraceDword(0x101);

       storeTraceDword(tcb->Rsyn); 
       storeTraceDword(tcb->Rsyna); 
       storeTraceDword(tcb->Rack); 
       storeTraceDword(tcb->Rget); 
       storeTraceDword(tcb->Rgeta); 
       storeTraceDword(tcb->Rdata); 
       storeTraceDword(tcb->Rfina); 
       storeTraceDword(tcb->Rfinaa); 
       */
       }
   }
   ...
   if (TRACE_RECORD_FLAG == 1) 
   {
       /*
    storeTraceDword(TCBRecordNum); 
    storeTraceDword(tcb->CURRENTTASK); 
    storeTraceDword(tcb->PORT[0]); 
    storeTraceDword(tcb->PORT[1]); 
    storeTraceDword(tcb->IP[0]); 
    storeTraceDword(tcb->IP[1]); 
    storeTraceDword(tcb->IP[2]); 
    storeTraceDword(tcb->IP[3]); 
    */
   }
...
if( TCBRecordNum != -1 )
{
   ...
   {        
       /*
            ...
            storeTraceDword(tcb->PORT[1]); 
            storeTraceDword(tcb->state); 
            storeTraceDword(io.AOAgetTimer()); 
            storeTraceDword(tcb->CURRENTTASK); 
            storeTraceDword(0); 
            */
       NoOfResets++; 
     ...
     if (TRACE_RECORD_FLAG)
     {
       ...
       storeTraceDword(tcb->PORT[0]);
       storeTraceDword(tcb->PORT[1]);
       storeTraceDword(NoOfResets);
       storeTraceDword(0);
       storeTraceDword(0);
       storeTraceDword(0);
       storeTraceDword(0x99999999);
     }
				...
				char *macaddr, long timer, int currenttask)
{
	...
	unsigned long tempseq=0;
		...
		else
		{
			...
			//DUMMY MESSAGE SENT TO FLUSH THE NIC TRANSMITTER
	...
	{
	     tempseq=charToseqnum(&TCPPack[8]);
 	     retcode =  SendMisc(SourceIP, &TCPPack[0], tempseq, TACK, macaddr, SYNACKTYPE, currenttask);
	}
...
//	         io.AOAPrintText("ii",Line19);
	...
	// 2. Check if the RST bit is set 
      ...
      {
          /*
       storeTraceDword(TCBRecordNumber);
       storeTraceDword(tcb->PORT[0]);
       storeTraceDword(tcb->PORT[1]);
       storeTraceDword(NoOfResets);
       storeTraceDword(0);
...
int TCPObj::SendMisc(char *destIP, char * destPort, char Flags, char *TargetMAC, int sendtype, int currenttask)
{
   ...
   //RKK Buffer Definitions 
	...
	send_buffer = send_buffer + 14 + 20 - ADDR_OFFSET;//add header before data  
        if (TCBRecordNum ==-1)
	      TCPPack_size = FormatTCPPacket(send_buffer, destIP, destPort, Flags, 0, 0, 0, data, 0, 0, currenttask); 		else
	{			
	      tcb = (TCBRecord*)(TCBBase + ( TCBRecordNum * sizeof(TCBRecord) ));
	      TCPPack_size = FormatTCPPacket(send_buffer, tcb->IP, tcb->PORT, Flags, tcb->RCVWND,
		                             tcb->SNDNXT,tcb->RCVNXT, 0, 0, 0, currenttask);  
...
int TCPObj::tcpSendN(int TCBRecordNum, int currenttask)
{
	...
	while (tcb->http_complete == 0)
       ...
       storeTraceDword(tcb->IP[2]); 
       storeTraceDword(tcb->IP[3]); 
       storeTraceDword(tcb->SRTT); 
       storeTraceDword(tcb->CURRENTTASK); 
       storeTraceDword(tcb->state); 
       storeTraceDword(HttpRequests); 
       */
...
//  writes into TRACE2 address 
//*************************************
int TCPObj::storeTraceDword(int dataw)

{
   ...
   addr = TRACE2 - 0x00110000;  //offset 

   if (trace2Count < MAX_TRACE2_COUNT)
	{
        addr = addr + trace2Count*4; 
        p1 = (int*)addr; 
        *p1 = dataw; 
        trace2Count++;  
	}

...
//-----------------------------------------------------------------------
//get the difference between clock ticks and return 32 bit value 
//  we assume that the difference is always small which fits in 32 bit value
//-----------------------------------------------------------------------
unsigned int AOAProtected::AOAgetClockTicksDiff(unsigned int hword1, unsigned int lword1, unsigned int hword2, 
        unsigned int lword2){
    int retcode = 0; 
    int i = 0; 
    int j = 0; 
    ...
    unsigned int mask [2]; //high and low words to shift bits  
    unsigned int bmask [2]; //high and low words to shift bits for borrow mask   
    unsigned int t1 = 0; 
    unsigned int t2 = 0; 
    unsigned int t3 = 0; 
    ...
    if (hword2 == 0 && hword1 == 0) 
        return (lword2 - lword1);   //most common case  
    ...
    {
        //substract 62 bit operands and return 32 bit result 
        //hword2 > 0 and lword2 could be smaller or greater than lword1
        ary1[0] = hword1; 
        ary1[1] = lword1; 
        ary2[0] = hword2; 
        ary2[1] = lword2; 
        k = 31;   //mask index  
        j = 0;    //low-high index  
        mask[0] = 0x00000001;   //mask bit in the last bit location  
        mask[1] = 0x00000001;   //mask bit in the last bit location  
        result[0] = 0; 
        result[1] = 0; 

        for (i=0; i<63; i++) 
        {
            if (j < 32)              //working on low word 
            {
            t3 = 0; 
            t2 = ary2[1] & mask[1]; //get the bit in t2 
            t1 = ary1[1] & mask[1]; //get the bit in t1 
            t3 = t2 ^ t1;           //exclusive or the bits 
            result[1] = result1 | t3; 
            borrow = t2 & t1;       //borrow bit 
            //-------------------------------------------------------
            //borrow logic
            // this logic will change bit positions in ary2 only
            //-------------------------------------------------------
            if (((t2 & mask[1]) != mask[1]) && ((t1 & mask[1]) == mask[1]))  //check 0 and 1 combination only
            {
            }
            j++; 
            k--;       
            mask[1] = mask[1] << 1;   //shift mask bit to left 
            if (k == 0) 
                k = 31;            //for next word 
            }
            else                    //work on high word  
            {
            t3 = 0; 
            t2 = ary2[0] & mask[0]; //get the bit in t2 
            t1 = ary1[0] & mask[0]; //get the bit in t1 
            t3 = t2 ^ t1;           //exclusive or the bits 
            result[0] = result0 | t3; 
            borrow = t2 & t1;       //borrow bit 
            if (borrow
            ...
            //borrow logic
            //-----------------------------
            mask[0] = mask[0] << 1;   //shift mask bit to left 
            }
...
unsigned long AOAProtected::AOAconvertChar4ToLong(char *fourchar)
{
    ...
    unsigned char c1; 
    int i=0; 
    AOAPrintText(fourchar, Line19+20);

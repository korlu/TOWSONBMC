...
bpbSectorsPerCluster    DB      0x20               ; 0x0D
bpbReservedSectors      DW      0x0006               ; 0x0E
bpbNumberOfFATs         DB      0x02               ; 0x10
bpbRootEntries          DW      0x0200               ; 0x11
bpbTotalSectors         DW      0               ; 0x13
bpbMedia                DB      0xF8               ; 0x15
bpbSectorsPerFAT        DW      0x00F5               ; 0x16
bpbSectorsPerTrack      DW      0x003F               ; 0x18
bpbHeadsPerCylinder     DW      0x00FF               ; 0x1A
bpbHiddenSectors        DD      0x00000020               ; 0x1C
bpbTotalSectorsBig      DD      0x011E97E0               ; 0x20

;;;;;;;;;;;;;;;;;;;
;; BPB ends here ;;
;;;;;;;;;;;;;;;;;;;

bsDriveNumber           DB      0x80               ; 0x24
...
; Your Data goes here!
    BOOT_MSG db 'Welcome to RKK/FACKERS BOOT!',10,13,0
    ACTIVITY db '. ',0
...
dapa:
	     	dw 0010h				; Packet size 16 bytes, this data structure size 
.count:		dw 0048				    ; Block count (no of sectors) RKK prcycle.exe size 
.off:		dw 0x0600			    ; Offset of buffer (memory location for destination)
.seg:		dw 0000h				; Segment of buffer (seg:offset)
.lbal:		dd 00000900h			; LBA (LSW) sectors STARTING SECTOR OF PRCYCLE.EXE RKK
.lbam:		dd 0				    ; LBA (MSW)
...
;		mov	dl, 80h
        ...
        jmp word 0x0:0x3900		;:0x0060				; jump to execute the sector! PRCYCLE

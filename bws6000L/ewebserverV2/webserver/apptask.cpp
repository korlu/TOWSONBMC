		...
		if(retcode < 0)
		{
			...
			switch(retcode)
			{
		         ...
		         case -11: 
			      ...
			      break;  		
		         case -12: 
			      io.AOAPrintText("TDL pointer is out of range TCPObj: -12", Line23); 
			      break; 
		         case -13: 
			      ...
			      break;  				      
		   	    case -29: 
			      io.AOAPrintText("TDL is FULL TCP:SendN: -29", Line23); 
			      break;  				      
		   	    case -30: 
			      io.AOAPrintText("TDL is FULL TCP:SendMisc: -30", Line23); 
			      break;  				      
		   	    case -31: 
			      io.AOAPrintText("Task do not match in tcb TCP:setFileParam: -31", Line23); 
			      break;  
		   	    case -32: 
			      io.AOAPrintText("Request is not GET TCP:OtherHandler: -32", Line23); 
			      break;  				      
		   	    case -33: 
...
{		
   ...
   io.AOAPrintText("TOTTIME", Line14+20); 
   io.AOAPrintText("RCVTIME", Line14+40); 
   io.AOAPrintText("HTTPTIME", Line14+60); 
   io.AOAPrintText("TIME   ", Line14+90); 
   ...
   printstarttimer = io.AOAgetTimer();  //initialize the timer  
   //Main task loop 
   cvalue = 0; 
   while (io.Application_Status != 2) 
	{
        totalstarttime = io.AOAgetTimer();
        //if ((io.AOAgetTimer() - systemstarttime) > 240010*10)
        
        if ((io.AOAgetTimer() - printstarttimer) >= 600*4000)  //600 sec timer to print 
        {
           cvalue = cvalue + 600;     //20 sec increment 
           ch = io.AOAgetCharacterBuff();
           printstarttimer = io.AOAgetTimer(); 
           ...
           io.AOAprintHex(totalcurtime, Line15+20); 
           io.AOAprintHex(rcvcurtime, Line15+40); 
           io.AOAprintHex(httpcurtime, Line15+60); 
           io.AOAprintHex(cvalue, Line15+80); 
        }
        
	    if (ch == 0x51) //Q command can be added here....
          {
                     ...
                     if (tcp.getResetFlag(tcbrnoc) == 1) 
		               {
                         ...
                         tcp.DeleteTCB(tcbrnoc);  //delete the tCB  
		                 continue;
	                   }
                     else if (tcp.getSuspendFlag(tcbrnoc) == 1 && tcp.getResumeFlag(tcbrnoc) == 1) 
		                {
                         ...
                         if (TRACE_RECORD_FLAG)
                            wttr.reset((taskid - MinTaskId), TASK_MASK_RESUME); 
		                 continue;
	                    }

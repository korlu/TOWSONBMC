...
char sendBuffer1[MAXBUFF];						//send buffer
char sendBuffer2[MAXBUFF];						//send buffer
char sendBuffer3[MAXBUFF];						//send buffer
char sendBuffer4[MAXBUFF];						//send buffer
char sendBuffer5[MAXBUFF];						//send buffer
char sendBuffer6[MAXBUFF];						//send buffer
char sendBuffer7[MAXBUFF];						//send buffer
char sendBuffer8[MAXBUFF];						//send buffer
char sendBuffer9[MAXBUFF];						//send buffer
char sendBuffer10[MAXBUFF];						//send buffer
int status1 =0;
int status2 =0;
int status3 =0;
int status4 =0;
...
char *interface= NULL;
char Path[100]="D:\\disk\\reu\\";				//we just use one interface
char Path2[180];
...
//
    ...
    while(1)
		...
		if (retval == SOCKET_ERROR) 
			{
            		fprintf(stderr,"A: recv() failed: error %d\n",WSAGetLastError());
            		closesocket(msgsock);
            		continue;
        		}	
...
EndTransfer:			    if(fileNumber>=totalFiles)
			...
			if (retval == SOCKET_ERROR) 
			{
            			fprintf(stderr,"B: recv() failed: error %d\n",WSAGetLastError());
            			//closesocket(msgsock);
            			goto EndTransfer;
        		}			
			else
			{	
				...
				CTimeSpan elapsedTime = endTime - startTime;	
					printf("\nHH:MM:SS %d:",(elapsedTime.GetHours());
					printf("%d:",(elapsedTime.GetMinutes());	
					printf("%d",(elapsedTime.GetSeconds());	 
...
Label1:	
				...
				if (retval == SOCKET_ERROR) 
				{
            			fprintf(stderr,"C: recv() failed: error %d\n",WSAGetLastError());
            			//closesocket(msgsock);
            			goto Label1;
        			}		
...
Label305:					
				...
				if (retval == SOCKET_ERROR) 
				{
            				fprintf(stderr,"D: recv() failed: error %d\n",WSAGetLastError());
            			//resend 303
            				goto Label303;
        			}			
				else

...
#define MAX_SENDRECEV_BUFF 2000
const int MAX = 5000;
...
extern "C"
{
	...
	int cgetTransmitInProg(int);               //pass IO_BASE  
	int cdnStall(int); 
	int cdnUnStall(int); 
	int cupStall(int); 
	int cupUnStall(int); 
...
//----------------------------------------------------------------------------------
	...
	int  enableTransmit(long);   //enable transmitter packets are in the buffer 
...
//----------------------------------------------------------------------------------
	...
	void getMAC(char*);
	void Close();
	int ARPInsertPkt(char* PACK,int PACK_Size, int PROTOCOL, char* Sender_HAdd, char* Target_HAdd, int currenttask);
	   //ARP send  
	int IPInsertPkt(char* PACK,int PACK_Size, int PROTOCOL, char* Target_HAdd, int currenttask);	
	   //IP data send 
	int SendPacket (long);//send packet for ARP and data  
	int CheckSendFlag ();//Check Send Flag for current Write DPD entry  
	int ReadData(char**,int,int*,int*, long*); 	
	int DPDFull(); 
